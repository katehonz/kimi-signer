...
impl DesktopSignerApp {
    ...
    pub fn browse_for_library(&mut self) {
        ...
        let filter = rfd::FileDialog::new()
            .add_filter("PKCS#11 Library", &["dll"]);
        
        #[cfg(target_os = "linux")]
        let filter = rfd::FileDialog::new()
            .add_filter("PKCS#11 Library", &["so"]);
        
        #[cfg(target_os = "macos")]
        let filter = rfd::FileDialog::new()
            .add_filter("PKCS#11 Library", &["dylib", "so"]);
        ...
        match self.signing_service.get_certificates() {
            Ok(token_certs) => {
                ...
                if self.certificates.is_empty() {
                    ...
                    self.status_message = format!("✓ Открити са {} сертификата", self.certificates.len());
                }
            }
            Err(e) => {
    ...
    pub fn perform_signing(&mut self) {
        ...
        let selected_thumbprint = self.selected_certificate.as_ref()
            .map(|c| c.thumbprint.clone())
            .unwrap_or_default();
        
        let token_cert = match self.token_certificates.iter()
        ...
        let output_path = match self.output_path.as_ref() {
            Some(dir) => {
                let file_name = self.selected_file.as_ref()
                    ...
                    .and_then(|s| s.to_str())
                    .unwrap_or("signed");
                dir.join(format!("{}{}", file_name, self.signature_type.extension()))
            }
            None => {
                ...
                let file_name = input_path.file_stem()
                    .and_then(|s| s.to_str())
                    .unwrap_or("signed");
                parent.join(format!("{}{}", file_name, self.signature_type.extension()))
            }
        };
        ...
        let signature_data = match self.signature_type {
            SignatureType::Attached => {
                // Create attached signature using CAdES
            ...
            SignatureType::Detached => {
                // Create detached signature using CAdES
        ...
        match std::fs::write(&output_path, &signature_data) {
            Ok(_) => {
                ...
                self.status_message = format!("✅ Документът е успешно подписан: {}", output_path.display());
            }
            Err(e) => {
...
impl eframe::App for DesktopSignerApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::bottom("bottom_panel").show(ctx, |ui| {
            ...
            let (icon, color) = if self.status_message.starts_with("✅") || self.status_message.starts_with("✓") {
                ("✅", egui::Color32::GREEN)
...
impl DesktopSignerApp {
        ...
        egui::Frame::group(ui.style()).show(ui, |ui| {
            ...
            if let Some(ref cert) = self.selected_certificate {
                ...
                ui.group(|ui| {
                    ...
                    ui.label(format!("Валиден от: {}", cert.valid_from.format("%d.%m.%Y %H:%M")));
                    ui.label(format!("Валиден до: {}", cert.valid_to.format("%d.%m.%Y %H:%M")));
                });
            }
        });
    }
    ...
    fn render_sign_button(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(ref result) = self.signature_result {
            ui.add_space(10.0);
            ui.colored_label(egui::Color32::GREEN, format!("✅ Създаден файл: {}", result));
        }
    }
